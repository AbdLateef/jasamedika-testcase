            ...
            <div class="md:w-1/4 flex flex-col gap-8">
                <div class="text-center md:text-start">
                    <h2 class="text-lg font-medium text-gray-900"><span
                            class="text-lg font-car font-normal text-gray-500">Nama:
                        </span>{{ $user->name }}</h2>
                    <h2 class="text-lg font-medium text-gray-900"><span
                    ...
                    <h2 class="text-lg font-medium text-gray-900"><span
                        ...
                        </span>{{ $user->drive_license }}</h2>
                    <h2 class="text-lg font-medium text-gray-900"><span
                            class="text-lg font-car font-normal text-gray-500">Alamat:
                        </span>{{ $user->address }}</h2>    
                    <h2 class="text-lg font-medium text-green-600"><span
                            class="text-lg font-car font-normal text-gray-500">
                                Reservasi aktif:</span>{{ $user->reservations->where('status', 'Active')->count() }}</h2>
                    <h2 class="text-lg font-medium text-yellow-300"><span
                            class="text-lg font-car font-normal text-gray-500">Reservasi
                            Pending: </span>{{ $user->reservations->where('status', 'Pending')->count() }}</h2>
                    <h2 class="text-lg font-medium text-Black"><span
                            class="text-lg font-car font-normal text-gray-500">Reservasi
                            Selesai: </span>{{ $user->reservations->where('status', 'Ended')->count() }}</h2>
                    <h2 class="text-lg font-medium text-red-600"><span
                            class="text-lg font-car font-normal text-gray-500">Reservasi
                            Batal: </span>{{ $user->reservations->where('status', 'Canceled')->count() }}</h2>
                        ...
                        <div class="m-3 p-1 md:w-[500px]">
                            ...
                            <div class="mt-4 flex justify-start md:gap-10 gap-5">
                                ...
                                <div class="flex gap-2 items-center">
                                    ...
                                    <p class="text-blue-400 font-semibold text-lg">{{ $reservation->total_price }} <span
